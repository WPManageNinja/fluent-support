...
class AgentController extends Controller
{
     ...
     *
     * No Agent type-hint here: route-model binding resolves inside the
     * permission callback, before any policy runs, which lets unauthenticated
     * callers probe agent ID existence (FS-PERM-001). Resolve after auth.
     * @param int|string $agent
     * @return array
     */
    public function resetAvatar($agent)
    {
        try {
            $agent = Agent::findOrFail((int) $agent);
            $agent->restoreAvatar();
    ...
    private function sanitizeAgentPayload(AgentCreateRequest $request, $includeEmail = false)
    {
        ...
        if ($request->has('agent_signature')) {
            // Already unslashed at the request boundary; sanitize only, so literal
            // backslashes in the signature survive.
            $data['agent_signature'] = wp_kses_post($request->get('agent_signature', ''));
        }
