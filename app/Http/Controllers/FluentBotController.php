...
class FluentBotController extends Controller
{
    ...
    /**
     * Reconnect to the ticket's in-flight FluentBot turn and stream its buffered SSE
     * to the browser. Used after a refresh or a conversation switch so a turn that
     * is still running upstream renders live instead of appearing as an empty panel.
     *
     * Emits `idle` (via the upstream) when nothing is buffered, so the client can
     * stop without special-casing.
     */
    public function resumeChatStream(Request $request, $id)
    {
        $ticketId = intval($id);
        $this->authorizeTicketAccess($ticketId);

        $meta = $this->getTicketMeta($ticketId, '_fluent_bot_chat_id');
        $chatId = $meta ? (string) $meta->value : '';

        // Only ever resume a chat this ticket owns, and only a well-formed id —
        // the id is interpolated into the upstream URL.
        if (!$chatId || !preg_match(self::CHAT_ID_PATTERN, $chatId)) {
            return $this->sendError(['message' => __('No conversation to resume.', 'fluent-support')], 404);
        }
        ...
        (new FluentBotService())->resumeChatStream($chatId, $productId);
