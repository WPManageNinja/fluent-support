...
{
    /**
     * Maximum number of customers accepted by a single bulk-delete request.
     */
    ...
    public function bulkDelete(Request $request, Customer $customer)
    {
        ...
        // Each id fans out into a full cascade delete (tickets, conversations,
        // attachments), so an unbounded batch means an unbounded request.
        $this->validate(['customer_ids' => $customerIds], [
            'customer_ids' => 'required|array|min:1|max:' . self::BULK_DELETE_LIMIT,
     ...
     * For a successful upload it's required to send file object, customer id and the user type(customer)
     *
     * No Customer type-hint here: route-model binding resolves inside the
     * permission callback, before any policy runs, which lets unauthenticated
     * callers probe customer ID existence (FS-PERM-001). Resolve after auth.
     * @param int|string $customer
     * @return array
     */
    public function resetAvatar($customer)
    {
        try {
            $customer = Customer::findOrFail((int) $customer);
    ...
    public function searchContact(Request $request)
    {
        ...
        // lone '*' would list every user on the site. Stripping it leaves
        // WP_User_Query doing an exact match.
