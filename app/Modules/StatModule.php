...
class StatModule
{
    ...
    public static function getAgentStat($agentId, $startDate = false, $endDate = false)
    {
        ...
        $newTickets = $access->applyMailboxRestrictionScope(
            Ticket::where('agent_id', $agentId)->where('status', 'new')
        )->count();

        //Get list of active ticket by agent
        $activeTickets = $access->applyMailboxRestrictionScope(
            Ticket::where('agent_id', $agentId)->where('status', 'active')
        )->count();

        //Get list of closed ticket by agent within a date range(default today)
        $closedTickets = $access->applyMailboxRestrictionScope(
        ...
        $responses = $access->applyMailboxRestrictionScopeViaTicket(
            Conversation::where('conversation_type', 'response')
                ->where('person_id', $agentId)
                ->whereBetween('created_at', [$startDate, $endDate])
        )->count();

        //Count the response in tickets within a date range(default today) for agent
        $interactions = $access->applyMailboxRestrictionScopeViaTicket(
            Conversation::where('person_id', $agentId)
    ...
    public static function getOverAllStats()
    {
        ...
        // Same mailbox boundary as the ticket list; these totals are agent-facing.
        $newTickets = $access->applyMailboxRestrictionScope(Ticket::where('status', 'new'))
            ->count();

        $activeTickets = $access->applyMailboxRestrictionScope(Ticket::where('status', 'active'))
            ->count();

        $closedTickets = $access->applyMailboxRestrictionScope(Ticket::where('status', 'closed'))
            ->count();
     ...
     */
    public static function getTodayStats($agentId = false, ?AgentTicketAccess $access = null)
    {
        $start = gmdate('Y-m-d 00:00.01');
        $end = gmdate('Y-m-d 23:59.59');

        $access = $access ?: new AgentTicketAccess();
        ...
        $closedTickets = $access->applyMailboxRestrictionScope(
            Ticket::where('status', 'closed')->whereBetween('resolved_at', [$start, $end])
        );
    ...
    public static function getAgentOverallStats($agentId)
    {
        ...
        $replies_count = (new AgentTicketAccess())->applyMailboxRestrictionScopeViaTicket(
            Conversation::where('person_id', $agentId)
        )->count();

        //Get the number of interactions/responses by agent with tickets
        $interactions_count = (new AgentTicketAccess())->applyMailboxRestrictionScopeViaTicket(
            Conversation::where('person_id', $agentId)
                ->where('conversation_type', 'response')
                ->groupBy('ticket_id')
        )->get()->count();
        ...
        $total_closed = (new AgentTicketAccess())->applyMailboxRestrictionScope(
            Ticket::where('agent_id', $agentId)->where('status', 'closed')
        )->count();
    ...
    public static function getActiveTicketsByProductStats()
    {
        ...
        foreach ($products as $product) {
            $result[$product->id] = [
                'title' => $product->title,
                'count' => static::countAwaitingTickets('product_id', $product->id, $access)
            ];
        }
     ...
     */
    public static function countAwaitingTickets($whereClause = null, $whereClauseValue = null, ?AgentTicketAccess $access = null)
    {
        // Must be a builder: ->where() on a bare model returns a new one, losing the scope.
        $ticket = Ticket::query();
        ...
        $ticket = ($access ?: new AgentTicketAccess())->applyMailboxRestrictionScope($ticket);
    ...
    {
        $stats = [];
        // One instance for the whole loop: resolving restrictions costs an agent and
        // meta read, and a fresh instance per agent would repeat it 44 times.
        $access = new AgentTicketAccess();

        Agent::get()->each(function ($agent) use (&$stats, $access) {
            $agentStat = static::getTodayStats($agent->id, $access);
            $waiting = static::countAwaitingTickets('agent_id', $agent->id, $access);
