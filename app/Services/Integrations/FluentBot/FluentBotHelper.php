...
class FluentBotHelper
{
    ...
    /**
     * Reconnect to an in-flight turn and stream its buffered SSE straight to the
     * browser. Writes directly to the output stream, so it returns nothing on
     * success; a credentials failure is emitted as an SSE `error` frame because the
     * caller has already sent SSE headers by this point.
     */
    public function resumeChatStream($chatId, $productId = null)
    {
        ...
        if (is_wp_error($credentials)) {
            echo "event: error\n";
            echo "data: " . wp_json_encode(['error' => $credentials->get_error_message()]) . "\n\n";
            flush();
            return;
        }

        $url = $this->apiBaseUrl() . '/bots/' . $credentials['botId'] . '/chats/' . $chatId . '/stream';

        (new FluentBotAPI($url, $credentials['apiKey']))->makeResumeStreamRequest();
    }
