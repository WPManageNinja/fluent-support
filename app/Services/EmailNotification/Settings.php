...
class Settings
{
    ...
    public function save($settingsKey, $settings)
    {
        ...
        if ($settingsKey == 'global_business_settings') {
            ...
            // The frontend only ever offers 3/7/15 — reject anything else
            // (a tampered request, or a stale/unexpected value) back to the
            // 15-day default rather than trusting the submitted value as-is.
            if (isset($settings['report_snapshot_retention']) && !in_array($settings['report_snapshot_retention'], ['3', '7', '15'], true)) {
                $settings['report_snapshot_retention'] = '15';
            }
        }
    ...
    private function getGlobalBusinessSettingsFields()
    {
        ...
        $fields = [
            ...
            'report_snapshot_retention' => [
                'wrapper_class' => '',
                'type'          => 'input-radio',
                'label'         => __('Snapshot Retention', 'fluent-support'),
                'options'       => [
                    ['id' => '3', 'label' => __('3 days', 'fluent-support')],
                    ['id' => '7', 'label' => __('7 days', 'fluent-support')],
                    ['id' => '15', 'label' => __('15 days', 'fluent-support')],
                ],
                'inline_help'   => __('Advanced Reports saves a snapshot of each report every 6 hours. Snapshots older than this many days are automatically deleted.', 'fluent-support'),
            ]
        ];
