...
#
# Resolution order:
#   1. $FCRM_WP_ROOT if set — an explicit override always wins
...
#   WP_ROOT="$(fcrm_resolve_wp_root "$PLUGIN_DIR")" || exit 2

fcrm_resolve_wp_root() {
  local plugin_dir="$1"
  local dir abspath

  # 1. Explicit override.
  if [ -n "${FCRM_WP_ROOT:-}" ]; then
    if [ ! -f "$FCRM_WP_ROOT/wp-load.php" ]; then
      echo "FCRM_WP_ROOT is set to '$FCRM_WP_ROOT' but no wp-load.php is there." >&2
      return 2
    fi
    printf '%s\n' "${FCRM_WP_ROOT%/}"
    return 0
  ...
  echo "Set it explicitly:  export FCRM_WP_ROOT=/path/to/wordpress" >&2
  return 2
}
