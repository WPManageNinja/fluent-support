...
* Added: Customer Portal prompts customers to confirm a changed account email via a signed verification link, and merges any unlinked customer record already collecting tickets at that address
* Added: `fluent_support/merging_customer_records` action so add-ons can move their own data when two customer records merge
* Improved: Keyboard-accessible ticket rows and responsive report tables, charts and toolbars
* Improved: Product dropdowns are alphabetized and searchable; fallback agent is now a searchable dropdown
* Improved: Half-hourly background tasks now run through Action Scheduler and reconcile automatically
* Improved: Per-agent report metrics use grouped queries instead of one query per agent, and mailbox restrictions are resolved once per request
* Improved: Consistent input and select heights in the admin UI; scoped date-range picker styles
* Improved: Report stat numbers now use thousand separators
* Improved: Translation coverage for admin strings — theme modes, ticket sidebar, report tabs, date shortcuts, workflow list and more
* Improved: (MCP) Silent close option for the close-ticket tool
...
* Fixed: Literal backslashes were stripped from ticket and reply content, including piped emails (Pro)
...
* Fixed: Long ticket titles overflowing instead of wrapping on the ticket view; ticket title typography
* Fixed: Inline attachments without a placeholder are now visible in the ticket thread (Pro)
